...
try {
  ...
  if ($firebaseExitCode -ne 0 -and -not ($firebaseOutput -match "BILLING_HTTP_API_FIRESTORE_VALIDATION_OK")) {
    throw "La validacion Firestore de Billing API fallo con codigo $firebaseExitCode"
  }

  if ($firebaseExitCode -ne 0) {
    Write-Warning "Firebase CLI devolvio $firebaseExitCode al cerrar el emulador, pero todas las aserciones terminaron correctamente."
  }
}
finally {
  Pop-Location
